...
class AlatModel extends Model
{
    ...
    public function getLaporan($id=null, $m=null, $lokasi=null){
        ...
        if($id != null){
            return $this->select('alat.nama, laporan.tanggal_periksa, laporan.jumlah_baik, laporan.jumlah_buruk, (laporan.jumlah_baik + laporan.jumlah_buruk) AS total_input, alat.jumlah, lokasi.nama_lokasi, laporan.catatan')->join('lokasi', 'alat.lokasi = lokasi.id')->join('alat', 'alat.id=laporan.id_alat', 'left')->where('laporan.id', $id)->findAll();
        }
    ...
    public function getTotalRasio($m, $lokasi)
    {
        ...
        // Final query
        $finalQuery = "SELECT SUM(rasio) AS total_rasio FROM ({$combinedQuery}) AS combined_rasio";
